...
Worker chỉ tự động xử lý thư mới sau mốc UID được tạo khi bật hoặc đổi Gmail. Chat và `/gmail search` vẫn có thể tìm trong toàn bộ lịch sử hộp thư. TaxSentry không tự xóa, archive hoặc đánh dấu thư là đã đọc.
...
| --- | --- |
| DOCX, XLSX, PPTX | Đọc trực tiếp |
| PDF | Trích xuất text; OCR khi cần |
...
TaxSentry lưu cấu hình, database, file tải về và báo cáo trong thư mục `~/.taxsentry`. Không commit App Password, Telegram Bot Token, database, file tải về hoặc báo cáo lên Git.
